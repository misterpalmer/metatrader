...
void CAccountProtector::Close_All_Positions()
{
    ...
    if (ArraySize(PositionsByProfit) > 0) // Is there anything to sort?
    {
        ...
        if ((CloseFirst == ENUM_CLOSE_TRADES_MOST_DISTANT_FIRST) || (CloseFirst == ENUM_CLOSE_TRADES_MOST_PROFITABLE_FIRST)) ArraySort(PositionsByProfit, WHOLE_ARRAY, 0, MODE_ASCEND); // Regardless of condition.
        else if ((CloseFirst == ENUM_CLOSE_TRADES_NEAREST_FIRST) || (CloseFirst == ENUM_CLOSE_TRADES_MOST_LOSING_FIRST)) ArraySort(PositionsByProfit, WHOLE_ARRAY, 0, MODE_DESCEND); // Regardless of condition.
...
void CAccountProtector::CheckAllConditions()
{
       ...
       )
    {
        ...
        double starting_balance = AccountInfoDouble(ACCOUNT_BALANCE) - daily_profit_loss_units - deposit_withdrawal;
        if (starting_balance != 0) daily_profit_loss_perc = daily_profit_loss_units / starting_balance * 100;
